...
def optimization_lev_1(qnode):
    """optimization w.r.t 1-qubit gates"""
    circuit_info = extract_info_from_qnode(qnode) # same with circuit_info
...
def optimization_lev_3(qnode):
    ...
    circuit_info = extract_info_from_qnode(qnode)
    qnode = selective_subcircuit_syntehsis(G, communities, circuit_info)

    # to be continued

    return qnode # returns an optimized qnode